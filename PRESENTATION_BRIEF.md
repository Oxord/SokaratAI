...
3. **Подбор пула вопросов** из `data/questions.json` (с fallback-набором, если в банке недобор). Размер пула равен `max_q` — верхней границе, рассчитанной `compute_bounds(level, num_skills)` (база по уровню + по скиллам, потолок 20).
4. **Адаптивный цикл (`min_q`–`max_q` итераций):**
   ...
   - после `min_q` отвечает `should_stop_interview`: при заданных скиллах — стопаем, как только каждый скилл получил ≥2 содержательных ответа; без скиллов — как только последние 3 ненулевых балла лежат в одной полосе (все ≥8 или все ≤3); иначе продолжаем до `max_q`.
